...
export default function Projects({ initialProjects }) {
    ...
    <div className="flex items-center gap-2">
      {isEditing ? (
        <>
          ...
          />
          <button onClick={handleSaveTitle} className="btn btn-sm btn-success">
            Save
          </button>
          <button
  ...
  const DropdownMenu = () => (
    <div className="dropdown dropdown-end">
      ...
      <ul
        tabIndex={0}
        className="menu dropdown-content z-[1] p-2 shadow bg-base-100 rounded-box mt-3"
      >
        <li
    ...
    >
      {(provided, snapshot) => (
        <div
          ...
          className={`card bg-base-100 p-3 shadow mb-3 transition-all ${
            snapshot.isDragging
              ? "opacity-50 scale-95 cursor-grab"
              : "cursor-pointer"
          }`}
      ...
      <Droppable droppableId={formatted} key={formatted}>
        {(provided, snapshot) => (
          <div
            ...
            className={`bg-base-200 rounded-box p-4 min-h-[300px] transition-all ${
